...
xf.createSelect = function(label, value, callback, self, formNode,selectionData) {
...
//arguments[0:content,1:'flow',2:'input-form-template']
...
xf.GridSection.prototype.render = function() {
	...
	var el = document.createElement('div');
	el.id = this.id;
	el.className = 'xf-section draggable-right';
	...
	for (var i = 0; i < this.row; i++) {
		...
		for (var j = 0; j < this.col; j++) {
			...
			html += '<td id="' + cellId + '" class="draggable-drop ' + cellClassName + '" width="' + (100 / this.col) + '%">&nbsp;'
				+ '</td>';
		}
		html += '</tr>';
	}
...
xf.GridSection.prototype.addRow = function() {
	...
	for (var i = 0; i < this.col; i++) {
		var td = document.createElement('td');
		td.id = tr.id + '-' + i;
		td.className = 'xf-cell-right xf-cell-bottom draggable-drop';
		if (i == 0) {
			td.className += ' xf-cell-left';
		}
...
xf.field.TextField.prototype.display = function() {
	...
	parentNode.innerHTML = 
		'<div class="xf-handler">'
		+ '<input type="text" name="' + this.name + '" alias="' + this.alias + '" ' + (this.readOnly ? 'readOnly' : '') + ' value="' + (this.value ? this.value : '') + '" style="margin-bottom:0px;cursor:move;">'
		+ '</div>';
}
...
xf.field.Calculator.prototype.display = function() {
	...
	parentNode.innerHTML = 
		'<div class="xf-handler">'
		+ '<input type="text" name="' + this.name + '" alias="' + this.alias + '" resources="'+this.resources+'" ' + (this.readOnly ? 'readOnly' : '') + ' value="' + (this.value ? this.value : '') + '" style="margin-bottom:0px;cursor:move;">'
		+ '</div>';
}
...
xf.field.TextArea.prototype.display = function() {
	...
	parentNode.innerHTML = 
		'<div class="xf-handler">'
		+ '<textarea name="' + this.name + '" alias="' + this.alias + '" ' + (this.readOnly ? 'readOnly' : '') + ' style="margin-bottom:0px;cursor:move;">' + (this.value ? this.value : '') + '</textarea>'
		+ '</div>';
}
...
xf.field.Select.prototype.updateItems = function(value) {
	...
	var html = 
		'<div class="xf-handler">'
		+ '<select name="' + this.name + '" alias="' + this.alias + '" ' + (this.readOnly ? 'disabled' : '') + ' style="margin-bottom:0px;cursor:move;width:150px;">';
	if(this.items){
...
xf.field.Radio.prototype.updateItems = function(value) {
	...
	for (var i = 0; i < array.length; i++) {
		var item = array[i];
		html += '<input type="radio" name="' + this.name + '" alias="' + this.alias + '" value="' + item + '" ' + (this.readOnly ? 'disabled' : '') + ' ' + (this.value == item ? 'checked' : '') + ' style="margin:0px;cursor:move;">';
		html += item;
	}
...
xf.field.Checkbox.prototype.updateItems = function(value) {
	...
	for (var i = 0; i < array.length; i++) {
		...
		html += '<input type="checkbox" name="' + this.name + '" alias="' + this.alias + '" value="' + item + '" ' + (this.readOnly ? 'disabled' : '') + ' ' + (checkedItem ? 'checked' : '') + ' style="margin:0px;cursor:move;">';
		html += item;
	}
	parentNode.innerHTML = html + '</div>';
}
...
xf.field.DatePicker.prototype.display = function() {
	...
	parentNode.innerHTML = 
		'<div class="xf-handler">'
	    + '<div style="padding: 0px;margin-bottom:0px;" class="input-append'+ (this.readOnly ? '': ' datepicker date') + '">'
	    + '<input type="text" name="' + this.name + '" alias="' + this.alias + '" '+ (this.readOnly ? 'readOnly="" ' : '') + 'style="background-color:white;cursor:default;" ' + (this.readOnly ? 'readOnly' : '') + ' value="' + (this.value ? this.value : '') + '">'
...
xf.field.UserPicker.prototype.display = function() {
	...
	parentNode.innerHTML = 
		'<div class="xf-handler">'
		//+ '<div style="padding-left: 0px;margin-bottom:0px;" class="input-append">'
		+ '<input type="text" name="' + this.name + '" alias="' + this.alias + '" '+ (this.readOnly ? 'readOnly="" ' : '') + 'class="userPicker" value="' + (this.value ? this.value : '') + '" style="margin-bottom:0px;cursor:move;">'
		+ '<span class="add-on" style="padding-top: 2px; padding-bottom: 2px;"><i style="cursor:pointer;" class="icon-user '+(this.readOnly ? '':' userPickerBtn')+'"></i></span>'
		//+ '</div>'
		+ '</div>';
}
...
xf.field.UserSelection.prototype.display = function() {
	...
	//控件拖动初始化
	var html ='<div class="xf-handler UserSelection">'
		 //+ '<div style="padding:0px;margin-bottom:0px;" class="input-append">'
		 + '<select name="'+this.name+'~'+xf_f_name_prefix+'us_r~@0'+'" ' + (this.readOnly ? 'disabled' : '') + ' style="margin-bottom:0px;cursor:move;width:150px;">';
		 ...
		 html += '<input type="text" name="'+this.name+'~'+xf_f_name_prefix+'us_u~@1'+'" alias="' + this.alias +'" '+ (this.readOnly ? 'readOnly="" ' : '') + 'required='+this.required+' ' +'reportFlag='+this.reportFlag+' ' +' class="userPicker"  style="margin-bottom:0px;cursor:move;">';
	...
	//debugger;
	//var userSelValArray = this.value;//?eval("("+this.value+")"):undefined;
	//if the collections has value
	if(this.value){
		html = '';
...
xf.field.UserSelection.prototype.loopItem = function(userSelValGroup) {
	...
	var html ='<div class="xf-handler">'
		 //+ '<div style="padding-left: 0px;margin-bottom:0px;" class="input-append">'
		 + '<select name="'+userSelValGroup[0].innerName+'"' + (this.readOnly ? 'disabled' : '') + ' style="margin-bottom:0px;cursor:move;width:150px;">';
		 if(this.rolesArr && this.rolesArr.length>0){
			for (var i = 0; i < this.rolesArr.length; i++) {
				var item = this.rolesArr[i];
		 ...
		 html += '</select>';
		 html += '<input type="text" name="'+userSelValGroup[1].innerName+'" '+ (this.readOnly ? 'readOnly="" ' : '') + 'required='
		 		+this.required+' class="userPicker" value="' + (user1?user1:'') 
		 		+ '" style="margin-bottom:0px;cursor:move;">';
		 
		 html += '<span class="add-on" style="padding-top: 2px; padding-bottom: 2px;border-radius: 0 4px 4px 0;"><i style="cursor:pointer;" class="icon-user '+(this.readOnly ? '':' userPickerBtn')+'"></i></span>';
		 //html += '<span '+(this.readOnly ? '':'class="userselectionAdd"')+' style="cursor: pointer;padding: 2px;">+</span>';
		 //html += '<span '+(this.readOnly ? '':'class="userselectionRemove"')+' style="cursor: pointer;padding: 2px;">-</span>';
		 //html += '</div>'
		 html += '</div>';
...
xf.field.ProSelection.prototype.updateItems = function(value) {
	...
	var html = 
		'<div class="xf-handler">'
		+ '<select name="' + this.name + '" alias="' + this.alias + '" ' + (this.readOnly ? 'disabled' : '') + 'class="xf-proselection" style="margin-bottom:0px;cursor:move;width:150px;">'
		+ '</select>'
		+ '</div>';
	parentNode.innerHTML = html;
}
